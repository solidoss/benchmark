...
struct AsyncClientCall {
    ...
    enum {
        ...
        STREAM_DONE_WRITE,
        STREAM_DONE
    } stage_ = STREAM_DONE_CONNECT;

    AsyncClientCall(TokenizerClient &_rclient):rclient_(_rclient){}

    AsyncClientCall(AsyncClientCall &&_other):rclient_(_other.rclient_), response_reader(std::move(_other.response_reader)), stream_rw_(std::move(_other.stream_rw_)){}
};
...
public:
    ...
    ): rctx_(_rctx), line_index_(_line_index), loop_count_(_loop_count){
